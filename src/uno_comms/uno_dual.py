...
class UnoDual:
    ...
    def connect(self) -> None:
        ...
        (e.g. the top Arduino isn't plugged in), leave self._ser as None
        and return without raising. The rest of the class already no-ops
        or raises a clear error when _ser is None, so callers can keep
        operating with one Arduino missing."""
        ...
        try:
            ...
            self._ser = None
            print(f"[UnoDual] Top Arduino not connected on {self.port}: {exc}")
            return
